...
- (void)setHuishouM:(IOSChoHuishouM *)huishouM{
    ...
    if (huishouM.isSelect) {
        [self.selelctButton setBackgroundImage:ImageNamed(@"IOSChoosedIcon") forState:0];
    }else{
        [self.selelctButton setBackgroundImage:ImageNamed(@"UISDuihao") forState:0];
    }
    self.NameLabel.text = huishouM.mateName;
    self.labelOne.text = kStringFormat(@"领取单号:%@",huishouM.mateId);
    self.LabelTwo.text = kStringFormat(@"领取时间:%@",huishouM.getTime);
    [self.UserHeaderImageV sd_setImageWithURL:[NSURL URLWithString:kStringFormat(@"%@%@",AppServerURL,huishouM.getUserPhoto)] placeholderImage:ImageNamed(@"placeholder")];
    self.userNameLabel.text = huishouM.getUserName;
