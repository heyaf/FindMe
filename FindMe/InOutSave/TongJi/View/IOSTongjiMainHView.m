...
#define kbtnW ((KDeviceWith-10-60-10-100)/4)
...
-(void)setSubViews{
    ...
    UILabel *dateLabel = [self createLabelFrame:CGRectMake(KDeviceWith-10-40-10-120, 10, 120, 24) textColor:IOSMainColor font:kFONT(12)];
...
-(UIView *)cellHeaderView{
    if (!_cellHeaderView) {
        _cellHeaderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kbtnW*4, 44)];
...
-(void)addHeaderSubViews{
    NSArray *titleArr = @[@"今日",@"昨日",@"近七日",@"近30天"];
    for (int i =0; i<titleArr.count; i++) {
        UIButton *but = [UIButton buttonWithType:0];
        but.frame = CGRectMake(kbtnW*i, 0, kbtnW, 44);
        [but setTitle:titleArr[i] forState:0];
        [but setTitleColor:IOSTitleColor forState:0];
        [but setBackgroundImage:[UIImage imageWithColor:RGBA(250, 250, 250, 1) size:CGSizeMake(kbtnW, 44)] forState:0];
