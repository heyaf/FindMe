...
-(void)getListDatawithType:(NSInteger)type{
    ...
    [[AFNetHelp shareAFNetworking] postInfoFromSeverWithStr:url body:MutparamDic sucess:^(id responseObject) {
        if ([AowString(responseObject[@"code"]) isEqualToString:@"1"]) {
            ...
            self.dataSource = [IOSInStoreModel arrayOfModelsFromDictionaries:datdic[@"list"] error:nil];
...
-(NSInteger)managernum1{
    NSInteger count = 0;
    for (IOSInStoreModel *listModel in self.dataSource) {
//            count+=listModel.num;
        count++;
    }
    return count;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        [cell setinstoredanHeadView:self.headerDic];
        return cell;
    }
    IOSGodsDetailTBCell *cell = [tableView dequeueReusableCellWithIdentifier:@"IOSGodsDetailTBCell"];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    IOSInStoreModel *instoreModel = [self manageChooseDate][indexPath.row];
    cell.instoreM = instoreModel;
    return cell;
}
