...
-(void)getListDatawithType:(NSInteger)type{
    ...
    [[AFNetHelp shareAFNetworking] postInfoFromSeverWithStr:url body:MutparamDic sucess:^(id responseObject) {
        if ([AowString(responseObject[@"code"]) isEqualToString:@"1"]) {
            ...
            self.dataSource = [IOSOutStoreM arrayOfModelsFromDictionaries:datdic[@"list"] error:nil];
...
-(NSString *)managerPrice{
    CGFloat count = 0.00;
    for (IOSOutStoreM *listModel in self.dataSource) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        [cell setoutstoredanHeadView:self.headerDic];
        return cell;
    }
    IOSGodsDetailTBCell *cell = [tableView dequeueReusableCellWithIdentifier:@"IOSGodsDetailTBCell"];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    IOSOutStoreM *instoreModel = [self manageChooseDate][indexPath.row];
    cell.outstoreM = instoreModel;
    return cell;
}
