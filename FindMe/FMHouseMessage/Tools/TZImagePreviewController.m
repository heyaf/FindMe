...
- (void)doneButtonClick {
    NSURL *photourl = _photos[_currentIndex];
    if ([photourl isKindOfClass:[NSNull class]]) {
        return;
    }
    NSString *suffix = [[photourl.absoluteString.lowercaseString componentsSeparatedByString:@"."] lastObject];
    if ([suffix isEqualToString:@"mp4"]) {
            NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
            NSString *documentsDirectory = [paths objectAtIndex:0];
            AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
            NSString  *fullPath = [NSString stringWithFormat:@"%@/%@", documentsDirectory, @"yuAnKu.mp4"];
            ...
            NSData *data = [NSData dataWithContentsOfURL:_photos[_currentIndex]];
...
- (void) savedPhotoImage:(UIImage*)image didFinishSavingWithError: (NSError *)error contextInfo: (void *)contextInfo {
    if (error) {
        ...
        [self showHint:@"视频保存失败"];
    }
    else {
        NSLog(@"保存视频成功");
        [self showHint:@"视频保存成功"];
    }
}
//保存图片的方法
