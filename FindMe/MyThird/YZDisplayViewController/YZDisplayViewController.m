...
- (UIColor *)selColor
{
    if (_selColor == nil) self.selColor = RGBA(235, 96, 59, 1);
...
- (UIView *)underLine
{
    if (_underLine == nil) {
        ...
        UIView *gayKuai = [[UIView alloc] initWithFrame:CGRectMake(0, YZTitleScrollViewH, KDeviceWith, 1)];
        gayKuai.backgroundColor = RGBA(245, 245, 245, 1);
        [self.contentView addSubview:gayKuai];
...
- (void)viewDidLayoutSubviews {
    ...
    if (_isInitial == NO) {
        ...
        CGFloat titleY = self.navigationController.navigationBarHidden == NO ?statusH+44:statusH;
...
- (void)setUpTitleWidth {
    ...
    for (NSString *title in titles) {
        ...
        if (self.isXianTT) {
            width = titleBounds.size.width /2;
        } else {
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (label.text.length > 7) {
         label.attributedText = [self recombinePrice: [label.text substringFromIndex:5] withYuanSize:10 orderPrice: [label.text substringToIndex:5] withXianSize:17];
        }
        ...
        if (_titleColorGradientStyle == YZTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要
            ...
            labelX =  YZScreenW/2-1.5*labelW+(i * labelW);
        }
...
- (NSAttributedString *)recombinePrice:(NSString *)CNPrice withYuanSize:(NSInteger )yuanSize orderPrice:(NSString *)unitPrice withXianSize:(NSInteger )xianSize
{
    ...
    NSAttributedString *string1 = [[NSAttributedString alloc] initWithString:unitPrice attributes:@{ NSFontAttributeName : [UIFont boldSystemFontOfSize:xianSize]}];
    NSAttributedString *string2 = [[NSAttributedString alloc] initWithString:CNPrice attributes:@{NSForegroundColorAttributeName : [UIColor lightGrayColor],NSFontAttributeName : [UIFont boldSystemFontOfSize:yuanSize] , NSStrikethroughColorAttributeName : [UIColor lightGrayColor]}];
